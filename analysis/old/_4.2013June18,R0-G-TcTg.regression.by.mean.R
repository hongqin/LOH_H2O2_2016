#2013 June 18
#  lm( Tc.vs.Tg ~ ln(R0) + G + ln(R0)*G )

#2013 May 29
#  lm() Tc.vs.Tg ~ ln(R0) + G )

#2012Feb25, Tg.vs.Tc ~ ln(R0) + G
# Partial correlations are all negative. Based on my realibity model, it argues that reverse of Tg.vs.Tc ~ robustness?!
# Tg/Tc is a measure of ability to maintian recombiation rate during aging
# Tc/Tg measure how long cells can live despite of the increased recombination rate. 
...
summary( lm( tb$Tg.vs.Tc ~ tb$G*log(tb$R0)) )  #negative correlation
summary( lm( 1/tb$Tg.vs.Tc ~ tb$G*log(tb$R0)) ) #positive correlation

 summary( lm( tb$G ~ log(tb$R0) + tb$Pbt0 + tb$L0.all + tb$Tg.vs.Tc  ) ) 
 summary( lm( tb$G ~ log(tb$R0) + tb$Pbt0 + tb$L0.all + tb$Tc.vs.Tg  ) ) 
 summary( lm( tb$Tc.vs.Tg ~ log(tb$R0) + tb$Pbt0 + tb$L0.all + tb$G   ) ) 
 
 summary( lm( tb$Tc.vs.Tg ~ log(tb$R0) + tb$G + tb$G:log(tb$R0) ) )  #20130618 change, p=0.0776
  plot( tb$Tc.vs.Tg ~ log(tb$R0) )
  plot( tb$Tc.vs.Tg ~ tb$G )
...
title("p=0.025, Strehler-Mildvan, natural isolates")
