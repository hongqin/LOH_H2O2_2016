...
FileList = list.files( path="../data.H2O2-LOH/", pattern="YPS128");  FileList; 
filename = 'YPS128,DEC9,2011.csv';
#filename = "YPS128,20110825,H2O2LOH.csv";
#filename = "YPS128,H2O2LOH_Nov14,2011.modified.csv";
fullFileName = paste('../data.H2O2-LOH/',filename, sep='');
mylabel = 'YPS128 2011 Dec 9'
...
model2 = function(v) {  1/( 1 + ( t / v )^ 3) }  
fm.s <- gnls( s ~ model2(v) , start = list( v = 0.01)  ); 
fm.s
fit.s = logistical.viability ( fm.s$coefficients[1], 2.5, t2 );

#model1 = function(v,w) {  1/( 1 + ( t / v )^ w ) }  
#fm.s <- gnls( s ~ model1(v,w) , start = list( v = 0.01, w=3)  );
...
b0.5.min = sum(b0.5.tmp[1])
b0.5.tmp = rev(b0.5.tmp); 
b0.5.max = sum(b0.5.tmp[1:5])/5 #account for increasing trend

modelBlack0.5B = function(v) { b0.5.max - (b0.5.max-b0.5.min)/(1 + (t/v)^10) }
fm.b0.5 <- gnls( b0.5 ~ modelBlack0.5B(v) , start = list( v = 0.00125) ); fm.b0.5
fit.b0.5 = logistical.black(b0.5.max, b0.5.min, fm.b0.5$coef[1], 2, t2 )
fit.b0.5 = logistical.black(b0.5.max, b0.5.min, 0.00125, 5, t2 )
...
#fm.b0.5 <- gnls( b0.5 ~ modelBlack0.5(v,w) , start = list( v = 0.001, w=5)  );   fm.b0.5
...
mtext( "Cb0.5",side=1,at=c(Cb0.5*3),line=-1, cex=0.8 );
