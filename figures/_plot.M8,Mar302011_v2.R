...
FileList = list.files( path="../data.H2O2-LOH/", pattern="M8");  FileList; 
filename = 'M8,Mar302011.Effectof H2O2onLOH.csv';
fullFileName = paste('../data.H2O2-LOH/',filename, sep='');
mylabel = 'M8 2011 Mar 30'
...
b.tmp = rev(b.tmp); 
b.max = sum(b.tmp[1:2])/2
  
modelBlack = function(v, w) { b.max - (b.max-b.min)/(1 + (t/v)^w) }
modelBlack1 = function(v) { b.max - (b.max-b.min)/(1 + (t/v)^0.5) }
# 
# fm.b <- gnls( b ~ modelBlack1(v) , start = list( v = 0.05) ); fm.b 
fm.b <- gnls( b ~ modelBlack(v,w) , start = list( v = 0.025, w= 2  )); fm.b
...
b0.5.max = sum(b0.5.tmp[1:2])/2 #account for increasing trend
...
fm.b0.5 <- gnls( b0.5 ~ modelBlack0.5(v,w) , start = list( v = 0.01, w=0.8)  );   fm.b0.5
