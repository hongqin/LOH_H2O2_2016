...
FileList = list.files( path="../data.H2O2-LOH/", pattern="M5");  FileList; 
#filename = 'M5,H202.022511.csv';
filename = 'M5,08162011.H2O2.LOH.csv';

fullFileName = paste('../data.H2O2-LOH/',filename, sep='');
mylabel = 'M5 2011 August 16'
...
mtext( "Cv",side=1,at=c(Cv*0.95), line=-1, cex=0.8 );
box()
title(mylabel)
...
b0.5.min = sum(b0.5.tmp[1])/1.1
b0.5.tmp = rev(b0.5.tmp); 
b0.5.max = sum(b0.5.tmp[1:4])/4 #account for increasing trend
...
fit.b0.5 = logistical.black(b0.5.max, b0.5.min, fm.b0.5$coef[1], 1.5, t2 )
...
#fm.b0.5 <- gnls( b0.5 ~ modelBlack0.5(v,w) , start = list( v = 0.0012, w=1.5)  );   fm.b0.5
...
mtext( "Cb0.5",side=1,at=c(Cb0.5+0.01),line=-1, cex=0.8 );
