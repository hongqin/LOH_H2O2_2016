...
FileList = list.files( path="../data.H2O2-LOH/", pattern="SGU");  FileList; 
filename = 'SGU57,Jan12,2012.modified.csv';
fullFileName = paste('../data.H2O2-LOH/',filename, sep='');
mylabel = 'SGU57 2012 January 12'
...
fm.s <- gnls( s ~ model1(v,w) , start = list( v = 0.01, w=0.5)  );
t2 = seq(0,1,by=0.001)
fit.s = logistical.viability ( fm.s$coefficients[1], fm.s$coefficients[2], t2 );

#model2 = function(v) {  1/( 1 + ( t / v )^1 ) }  
#fm.s <- gnls( s ~ model2(v) , start = list( v = 0.05)  );fm.s
#fit.s = logistical.viability ( fm.s$coefficients[1], 1, t2 );
Cv = fm.s$coefficients[1]

#estimate starting values
b.tmp = sort( b[ b>0 ] );
b.min = sum(b.tmp[1])*1.1
b.tmp = rev(b.tmp); 
b.max = sum(b.tmp[1:3])/3.3
...
fm.b <- gnls( b ~ modelBlack(v,w) , start = list( v = 0.015, w= 1.2  )); fm.b
...
pdf(paste("_20131216", filename, "pdf", sep="."), width=5,height=8); 
...
b0.5.min = sum(b0.5.tmp[1])/1.5
b0.5.tmp = rev(b0.5.tmp); 
b0.5.max = sum(b0.5.tmp[1:4])/4.9 #account for increasing trend
...
fm.b0.5 <- gnls( b0.5 ~ modelBlack0.5(v,w) , start = list( v = 0.009, w=1.5)  );   fm.b0.5
...
mtext( "Cb0.5",side=1,at=c(Cb0.5+0.004),line=-1, cex=0.8 );
